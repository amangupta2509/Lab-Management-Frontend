    ...
    >
      ...
      {dashboardData?.activeSession && (
        <TouchableOpacity style={styles.activeSessionCard}>
          ...
          <Text style={styles.activeSessionSubtext}>
            Started: {new Date(dashboardData.activeSession.start_time).toLocaleTimeString()}
          </Text>
        </TouchableOpacity>
      )}
      ...
      <View style={styles.statsContainer}>
        <View style={styles.statCard}>
          ...
          <Text style={styles.statValue}>{dashboardData?.weekStats.workHours || '0'}h</Text>
          <Text style={styles.statLabel}>This Week</Text>
        </View>
        <View style={styles.statCard}>
          <Ionicons name="calendar-outline" size={32} color="#4caf50" />
          <Text style={styles.statValue}>{dashboardData?.upcomingBookings.length || 0}</Text>
          <Text style={styles.statLabel}>Upcoming</Text>
        </View>
        <View style={styles.statCard}>
          <Ionicons name="bar-chart-outline" size={32} color="#ff9800" />
          <Text style={styles.statValue}>{dashboardData?.weekStats.sessions || 0}</Text>
          ...
          dashboardData.upcomingBookings.slice(0, 3).map((booking) => (
            <TouchableOpacity key={booking.id} style={styles.bookingCard}>
              <View style={styles.bookingHeader}>
                <Text style={styles.bookingEquipment}>{booking.equipment_name}</Text>
                <View
                  style={[
                    styles.statusBadge,
                ...
                >
                  <Text
                    style={[
                      styles.statusText,
                      { color: booking.status === 'approved' ? '#4caf50' : '#ff9800' },
                    ]}
                  >
                    {booking.status}
                  </Text>
