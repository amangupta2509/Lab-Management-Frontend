...
export default function SessionsScreen() {
  ...
  const loadData = async () => {
    try {
      ...
      // Load approved bookings for starting sessions
      const bookingsResponse = await bookingAPI.getMyBookings({
        status: "approved",
      });
      const today = new Date().toISOString().split("T")[0];

      // Filter bookings that are for today and don't have active sessions
      const available = bookingsResponse.data.bookings.filter((b: any) => {
        const bookingDate = new Date(b.booking_date)
          .toISOString()
          .split("T")[0];
        const hasSession = sessionsResponse.data.sessions.some(
          (s: UsageSession) => s.id === b.id && !s.end_time
        );
        return bookingDate === today && !hasSession && b.status === "approved";
      });
        ...
        ListEmptyComponent={
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyStateSubtext}>
              Start a session from your approved bookings
            </Text>
          </View>
        }
      />
          ...
          <View style={styles.modalContent}>
            ...
            <ScrollView style={styles.modalBody}>
              <Text style={styles.modalDescription}>
                Select an approved booking to start your session:
              </Text>
