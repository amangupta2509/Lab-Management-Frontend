...
export default function ActivityScreen() {
  ...
  const checkSignInStatus = async () => {
    try {
      ...
      const todayLog = response.data.activities.find(
        (a: any) => a.activity_date === today && !a.sign_out_time
      );
      setIsSignedIn(!!todayLog);
      setTodayActivity(todayLog || null);
    } catch (error) {
  ...
  const handleSignIn = async () => {
    try {
      // The backend automatically signs in when user makes any request
      // We'll make a simple API call to trigger the auto sign-in
      await activityAPI.getMyActivity();

      Alert.alert("Success", "Signed in to lab successfully");
      checkSignInStatus();
    } catch (error: any) {
      Alert.alert("Error", "Failed to sign in to lab");
    }
  };
